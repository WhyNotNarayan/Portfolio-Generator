...
const Card3: React.FC<TemplateProps> = ({ data }) => {
   ...
   // AESTHETIC VIBRANT ORANGE & DARK SLATE
   const primaryColor = '#F97316'; // Orange
   const softAccent = '#FDBA74'; // Soft Orange
   const bgColor = isDarkMode ? '#0F172A' : '#F8FAFC'; // Dark Background / Soft Slate
   const cardBg = isDarkMode ? '#1E293B' : '#FFFFFF'; // Darker Slate / White
   const textColor = isDarkMode ? '#F1F5F9' : '#0F172A'; // Text
   ...
   return (
      <div style={{
        ...
        .cta-outline { background: transparent; color: inherit; padding: 1.2rem 2.5rem; border-radius: 12px; border: 2px solid ${isDarkMode ? 'rgba(249,115,22,0.3)' : 'rgba(0,0,0,0.1)'}; font-size: 1.1rem; font-weight: 700; cursor: pointer; transition: 0.3s; }
        ...
        .profile-glow { border-radius: 50%; border: 8px solid ${primaryColor}11; box-shadow: 0 0 40px ${primaryColor}22; transition: all 0.5s ease; }
        .profile-glow:hover { box-shadow: 0 0 60px ${primaryColor}44; transform: scale(1.02); }

        .elite-card { background: ${cardBg}; border: 1px solid ${isDarkMode ? 'rgba(249,115,22,0.1)' : '#edf2f7'}; border-radius: 24px; padding: 3rem; transition: 0.3s; box-shadow: ${isDarkMode ? '0 10px 30px rgba(0,0,0,0.3)' : '0 10px 30px rgba(0,0,0,0.04)'}; }
        .elite-card:hover { transform: translateY(-10px); box-shadow: 0 20px 40px ${primaryColor}11; border-color: ${primaryColor}; }

        .skill-grid-card { background: ${cardBg}; border-radius: 20px; padding: 2.5rem; border-left: 5px solid ${primaryColor}; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); cursor: default; box-shadow: ${isDarkMode ? '0 10px 20px rgba(0,0,0,0.2)' : '0 10px 25px rgba(0,0,0,0.03)'}; }
        .skill-grid-card:hover { transform: translateY(-8px); box-shadow: 0 15px 40px ${primaryColor}${isDarkMode ? '22' : '11'} !important; border-left-width: 8px; }
      ...
      `}} />
         ...
         {/* Navigation */}
         <nav style={{ padding: '1.5rem 6%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 1000, background: isDarkMode ? 'rgba(15, 23, 42, 0.95)' : 'rgba(248, 250, 252, 0.95)', backdropFilter: 'blur(10px)', borderBottom: isDarkMode ? `1px solid ${primaryColor}11` : 'none' }}>
         ...
         {/* About Section */}
         <section id="about" style={{ padding: '8rem 6%', borderTop: `1px solid ${isDarkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'}`, background: isDarkMode ? '#0F172A' : '#ffffff' }}>
         ...
         {/* Services Section */}
         <section id="services" style={{ padding: '10rem 6%', background: isDarkMode ? '#0F172A' : '#ffffff' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
         ...
         {/* Experience Timeline */}
         <section id="experience" style={{ padding: '10rem 6%', background: isDarkMode ? '#0F172A' : '#f7fafc' }}>
         ...
         {/* Skills Section */}
         <section id="skills" style={{ padding: '10rem 6%', background: isDarkMode ? '#0F172A' : '#eff3f6' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
               <h2 style={{ fontSize: '4rem', fontWeight: 900, marginBottom: '6rem', textAlign: 'center', color: primaryColor, opacity: 0.9 }}>Skills & <span style={{ color: isDarkMode ? '#fff' : '#0F172A' }}>Expertise</span></h2>
                     ...
                     return renderedCategories.map((c, i) => (
                        <div key={i} className="skill-grid-card">
                           ...
                           <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                              {c.skills.map(s => (
                                 <span key={s} style={{ padding: '0.8rem 1.8rem', background: isDarkMode ? '#1e293b' : '#f3f4f6', borderRadius: '100px', fontWeight: 700, fontSize: '1rem', border: `1px solid ${isDarkMode ? primaryColor + '33' : 'transparent'}`, transition: '0.3s' }}>
                                    {s}
                                 </span>
                              ))}
                           </div>
         ...
         <section id="certifications" style={{ padding: '10rem 6%', borderTop: isDarkMode ? `1px solid ${primaryColor}11` : '1px solid rgba(0,0,0,0.05)' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
         ...
         {/* Contact Section */}
         <section id="contact" style={{ padding: '10rem 6%', background: isDarkMode ? '#0F172A' : '#f7fafc', borderTop: isDarkMode ? `4px solid ${primaryColor}` : 'none' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
               <h2 style={{ fontSize: '3.5rem', fontWeight: 900, textAlign: 'center', marginBottom: '8rem', color: primaryColor }}>Get In <span style={{ color: isDarkMode ? '#fff' : '#0F172A' }}>Touch</span></h2>
                  ...
                  <div style={{ background: cardBg, borderRadius: '30px', padding: '3.5rem', boxShadow: isDarkMode ? '0 10px 40px rgba(0,0,0,0.5)' : '0 10px 40px rgba(0,0,0,0.1)' }}>
                     ...
                     <form action={`https://formsubmit.co/${data.email}`} method="POST" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <div>
                           ...
                           <input type="text" name="name" required placeholder="Your Name" style={{ width: '100%', padding: '1.2rem', borderRadius: '12px', background: isDarkMode ? '#0F172A' : '#f3f4f6', border: `1px solid ${isDarkMode ? primaryColor + '44' : 'transparent'}`, color: 'inherit', fontWeight: 600 }} />
                        </div>
                        <div>
                           <label style={{ fontSize: '0.9rem', fontWeight: 800, opacity: 0.8, display: 'block', marginBottom: '0.6rem' }}>Email</label>
                           <input type="email" name="email" required placeholder="Your Email" style={{ width: '100%', padding: '1.2rem', borderRadius: '12px', background: isDarkMode ? '#0F172A' : '#f3f4f6', border: `1px solid ${isDarkMode ? primaryColor + '44' : 'transparent'}`, color: 'inherit', fontWeight: 600 }} />
                        </div>
                        <div>
                           <label style={{ fontSize: '0.9rem', fontWeight: 800, opacity: 0.8, display: 'block', marginBottom: '0.6rem' }}>Subject</label>
                           <input type="text" name="subject" required placeholder="Subject" style={{ width: '100%', padding: '1.2rem', borderRadius: '12px', background: isDarkMode ? '#0F172A' : '#f3f4f6', border: `1px solid ${isDarkMode ? primaryColor + '44' : 'transparent'}`, color: 'inherit', fontWeight: 600 }} />
                        </div>
                        <div>
                           <label style={{ fontSize: '0.9rem', fontWeight: 800, opacity: 0.8, display: 'block', marginBottom: '0.6rem' }}>Message</label>
                           <textarea name="message" rows={5} required placeholder="Your Message" style={{ width: '100%', padding: '1.2rem', borderRadius: '12px', background: isDarkMode ? '#0F172A' : '#f3f4f6', border: `1px solid ${isDarkMode ? primaryColor + '44' : 'transparent'}`, color: 'inherit', fontWeight: 600 }}></textarea>
                        </div>
                  ...
                  <div style={{ background: cardBg, borderRadius: '30px', padding: '3.5rem', boxShadow: '0 10px 40px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                     ...
                     <div style={{ marginTop: 'auto', borderRadius: '20px', overflow: 'hidden', border: `1px solid ${primaryColor}22`, height: '250px' }}>
                        <iframe 
                           ...
                           height="100%" 
                           frameBorder="0" 
                           style={{ border: 0, filter: isDarkMode ? 'contrast(1.2) brightness(0.8) hue-rotate(180deg) invert(0.9)' : 'none' }} 
