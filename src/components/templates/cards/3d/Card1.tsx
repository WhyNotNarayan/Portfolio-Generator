...
const Card1: React.FC<TemplateProps> = ({ data }) => {
   ...
   const bgColor = isDarkMode ? '#030712' : '#f9fafb';
   ...
   return (
      <div style={{
        ...
        .btn-resume { background: ${primaryColor}; color: white; padding: 0.6rem 1.4rem; border-radius: 8px; border: none; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 0.8rem; transition: 0.3s; }
        .btn-resume:hover { transform: translateY(-2px); box-shadow: 0 4px 12px ${primaryColor}44; }

        .social-circle { width: 45px; height: 45px; border-radius: 50%; border: 1px solid ${primaryColor}44; display: flex; align-items: center; justify-content: center; color: ${primaryColor}; transition: all 0.3s; cursor: pointer; background: rgba(255,255,255,0.05); backdrop-filter: blur(5px); text-decoration: none; }
        .social-circle:hover { background: ${primaryColor}; color: white; transform: translateY(-5px); box-shadow: 0 8px 15px ${primaryColor}33; }

        .cta-primary { background: ${primaryColor}; color: white; padding: 1.2rem 2.5rem; border-radius: 12px; border: none; font-size: 1.1rem; font-weight: 700; cursor: pointer; transition: all 0.3s; }
        .cta-outline { background: transparent; color: inherit; padding: 1.2rem 2.5rem; border-radius: 12px; border: 2px solid ${isDarkMode ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)'}; font-size: 1.1rem; font-weight: 700; cursor: pointer; transition: 0.3s; backdrop-filter: blur(5px); }
        ...
        .elite-card { background: ${isDarkMode ? 'rgba(31, 41, 55, 0.4)' : 'rgba(255, 255, 255, 0.8)'}; backdrop-filter: blur(12px); border: 1px solid ${isDarkMode ? 'rgba(255,255,255,0.1)' : '#edf2f7'}; border-radius: 24px; padding: 3rem; transition: 0.3s; position: relative; z-index: 10; }
        .elite-card:hover { transform: translateY(-10px); box-shadow: 0 20px 40px rgba(99, 102, 241, 0.15); border-color: ${primaryColor}; }

        .skill-grid-card { background: ${isDarkMode ? 'rgba(31, 41, 55, 0.4)' : 'rgba(255, 255, 255, 0.8)'}; backdrop-filter: blur(12px); border-radius: 20px; padding: 2.5rem; border-left: 5px solid ${primaryColor}; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); cursor: default; position: relative; z-index: 10; }
        .skill-grid-card:hover { transform: translateY(-8px); box-shadow: 0 15px 40px ${primaryColor}33 !important; border-left-width: 8px; }
      ...
      `}} />
         ...
         {/* Navigation */}
         <nav style={{ padding: '1.5rem 6%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 1000, background: isDarkMode ? 'rgba(3, 7, 18, 0.8)' : 'rgba(249, 250, 251, 0.8)', backdropFilter: 'blur(20px)', borderBottom: isDarkMode ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
         ...
         <section className="hero-content" style={{ padding: '8rem 6%', maxWidth: '1400px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '4rem' }}>
            <div style={{ flex: 1 }}>
               <h1 style={{ fontSize: '5rem', fontWeight: 900, marginBottom: '0.5rem', letterSpacing: '-0.02em', textShadow: isDarkMode ? '0 0 20px rgba(99, 102, 241, 0.3)' : 'none' }}>
         ...
         {/* About Section */}
         <section id="about" style={{ padding: '8rem 6%', background: isDarkMode ? 'transparent' : '#ffffff' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
               <div style={{ display: 'grid', gridTemplateColumns: 'minmax(200px, 1fr) 2fr', gap: '6rem', alignItems: 'start' }}>
                  ...
                  <div className="elite-card" style={{ background: isDarkMode ? 'rgba(31, 41, 55, 0.6)' : '#fff' }}>
                  ...
                  {data.projects?.map((p, i) => (
                     <div key={i} className="elite-card">
                        ...
                        <div style={{ display: 'flex', gap: '0.8rem', flexWrap: 'wrap', marginBottom: '3rem' }}>
                           {p.tech?.split(',').map(t => <span key={t} style={{ background: primaryColor, color: '#fff', padding: '0.5rem 1rem', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 700 }}>{t}</span>)}
                        </div>
            ...
            <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
               ...
               {data.experience?.map((e, i) => (
                  <div key={i} style={{ display: 'flex', gap: '4rem', marginBottom: '6rem', paddingLeft: '3rem', borderLeft: `2px solid ${primaryColor}44`, position: 'relative' }}>
                     <div style={{ position: 'absolute', left: '-7px', top: '0', width: '12px', height: '12px', borderRadius: '50%', background: primaryColor, boxShadow: `0 0 10px ${primaryColor}` }}></div>
                     <div style={{ minWidth: '150px', fontWeight: 700, color: primaryColor }}>{e.duration}</div>
                     <div className="elite-card" style={{ padding: '2rem', flex: 1, background: isDarkMode ? 'rgba(31, 41, 55, 0.4)' : 'rgba(255, 255, 255, 0.8)' }}>
         ...
         <section id="skills" style={{ padding: '10rem 6%' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
               <h2 style={{ fontSize: '4rem', fontWeight: 900, marginBottom: '6rem', textAlign: 'center', color: primaryColor, opacity: 0.9 }}>Skills & <span style={{ color: isDarkMode ? '#fff' : '#111827' }}>Expertise</span></h2>
                     ...
                     return renderedCategories.map((c, i) => (
                        <div key={i} className="skill-grid-card">
                           ...
                           <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                              {c.skills.map(s => (
                                 <span key={s} style={{ padding: '0.8rem 1.8rem', background: isDarkMode ? 'rgba(0,0,0,0.3)' : '#f3f4f6', borderRadius: '100px', fontWeight: 700, fontSize: '1rem', border: `1px solid ${isDarkMode ? 'rgba(255,255,255,0.05)' : 'transparent'}`, transition: '0.3s' }}>
                                    {s}
                                 </span>
                              ))}
                           </div>
         ...
         <section id="contact" style={{ padding: '10rem 6%' }}>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '4rem' }}>
                  <div className="elite-card">
                     ...
                     <form action={`https://formsubmit.co/${data.email}`} method="POST" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <input type="hidden" name="_subject" value={`New 3D Portfolio Inquiry from ${data.name}!`} />
            ...
            <p style={{ marginTop: '10rem', opacity: 0.25, textAlign: 'center', fontSize: '0.8rem', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 900, userSelect: 'none', pointerEvents: 'none' }}>
               © {new Date().getFullYear()} @{data.name.toLowerCase().replace(/\s+/g, '')}. Created with <span style={{ color: primaryColor }}>PortGen 3D.</span>
            </p>
         </section>
      </div>
   );
