...
const Card4: React.FC<TemplateProps> = ({ data }) => {
   ...
   return (
      <div style={{
        ...
        .elite-card { background: ${isDarkMode ? '#1f2937' : '#fff'}; border: 1px solid ${isDarkMode ? 'transparent' : '#edf2f7'}; border-radius: 24px; padding: 3rem; transition: 0.3s; box-shadow: ${isDarkMode ? 'none' : '0 10px 30px rgba(0,0,0,0.04)'}; }
        .elite-card:hover { transform: translateY(-10px); box-shadow: 0 20px 40px ${isDarkMode ? 'rgba(0,0,0,0.2)' : 'rgba(139, 92, 246, 0.15)'}; border-color: ${primaryColor}; }

        .skill-grid-card { background: ${isDarkMode ? '#1f2937' : '#fff'}; border-radius: 20px; padding: 2.5rem; border-left: 5px solid ${primaryColor}; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); cursor: default; box-shadow: ${isDarkMode ? 'none' : '0 10px 25px rgba(0,0,0,0.03)'}; }
      ...
      `}} />
         ...
         {/* Navigation */}
         <nav style={{ padding: '1.5rem 6%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 1000, background: isDarkMode ? 'rgba(17, 24, 39, 0.9)' : 'rgba(249, 250, 251, 0.9)', backdropFilter: 'blur(10px)' }}>
