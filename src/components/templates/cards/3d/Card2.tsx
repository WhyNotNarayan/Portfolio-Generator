...
import Scene3D_2 from './Scene3D_2';
   ...
   return (
      <div style={{
         ...
         <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 0, pointerEvents: 'none' }}>
            <Scene3D_2 isDarkMode={isDarkMode} />
         </div>
        ...
        .btn-resume { background: ${primaryColor}; color: #0B0B0B; padding: 0.6rem 1.4rem; border-radius: 8px; border: none; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 0.8rem; transition: 0.3s; }
        .btn-resume:hover { transform: translateY(-2px); box-shadow: 0 4px 12px ${primaryColor}44; }

        .social-circle { width: 45px; height: 45px; border-radius: 50%; border: 1px solid ${primaryColor}44; display: flex; align-items: center; justify-content: center; color: ${primaryColor}; transition: all 0.3s; cursor: pointer; background: rgba(0,0,0,0.2); backdrop-filter: blur(5px); text-decoration: none; }
        .social-circle:hover { background: ${primaryColor}; color: #0B0B0B; transform: translateY(-5px); box-shadow: 0 8px 15px ${primaryColor}33; }

        .cta-primary { background: ${primaryColor}; color: #0B0B0B; padding: 1.2rem 2.5rem; border-radius: 12px; border: none; font-size: 1.1rem; font-weight: 700; cursor: pointer; transition: all 0.3s; }
        .cta-outline { background: transparent; color: inherit; padding: 1.2rem 2.5rem; border-radius: 12px; border: 2px solid ${isDarkMode ? 'rgba(212, 175, 55, 0.3)' : 'rgba(0,0,0,0.1)'}; font-size: 1.1rem; font-weight: 700; cursor: pointer; transition: 0.3s; backdrop-filter: blur(5px); }
        .cta-primary:hover { opacity: 0.9; transform: scale(1.02); }
        .cta-outline:hover { border-color: ${primaryColor}; color: ${primaryColor}; }

        .profile-glow { border-radius: 50%; border: 8px solid ${primaryColor}22; box-shadow: 0 0 40px ${primaryColor}33; transition: all 0.5s ease; position: relative; z-index: 10; }
        .profile-glow:hover { box-shadow: 0 0 60px ${primaryColor}55; transform: scale(1.02); }

        .elite-card { background: ${isDarkMode ? 'rgba(20, 20, 20, 0.6)' : 'rgba(255, 255, 255, 0.8)'}; backdrop-filter: blur(15px); border: 1px solid ${isDarkMode ? 'rgba(212, 175, 55, 0.1)' : '#edf2f7'}; border-radius: 24px; padding: 3rem; transition: 0.3s; position: relative; z-index: 10; }
        .elite-card:hover { transform: translateY(-10px); box-shadow: 0 20px 40px ${primaryColor}11; border-color: ${primaryColor}; }

        .skill-grid-card { background: ${isDarkMode ? 'rgba(20, 20, 20, 0.6)' : 'rgba(255, 255, 255, 0.8)'}; backdrop-filter: blur(15px); border-radius: 20px; padding: 2.5rem; border-left: 5px solid ${primaryColor}; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); cursor: default; position: relative; z-index: 10; }
        .skill-grid-card:hover { transform: translateY(-8px); box-shadow: 0 15px 40px ${primaryColor}22 !important; border-left-width: 8px; }

        section { position: relative; z-index: 10; scroll-margin-top: 80px; }
      ...
      `}} />
         ...
         {/* Navigation */}
         <nav style={{ padding: '1.5rem 6%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 1000, background: isDarkMode ? 'rgba(11, 11, 11, 0.8)' : 'rgba(255, 252, 240, 0.8)', backdropFilter: 'blur(20px)', borderBottom: isDarkMode ? '1px solid rgba(212, 175, 55, 0.1)' : 'none' }}>
         ...
         <section className="hero-content" style={{ padding: '8rem 6%', maxWidth: '1400px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '4rem' }}>
            <div style={{ flex: 1 }}>
               <h1 style={{ fontSize: '5rem', fontWeight: 900, marginBottom: '0.5rem', letterSpacing: '-0.02em', textShadow: isDarkMode ? `0 0 20px ${primaryColor}22` : 'none' }}>
            ...
            <div className="profile-container" style={{ flex: '0 0 500px', display: 'flex', justifyContent: 'center', position: 'relative' }}>
               <div className="profile-glow" style={{ width: '480px', height: '480px', overflow: 'hidden', border: `2px solid ${primaryColor}` }}>
               ...
               <div style={{ position: 'absolute', width: '550px', height: '550px', borderRadius: '50%', background: `radial-gradient(circle, ${primaryColor}22, transparent 70%)`, zIndex: 1, pointerEvents: 'none' }}></div>
            </div>
         </section>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '3rem' }}>
                  {data.services?.map((s, i) => (
                     <div key={i} className="elite-card">
                        ...
                        <p style={{ fontSize: '1.1rem', lineHeight: 1.7, opacity: 0.7, fontWeight: 500 }}>Delivering bespoke luxury digital solutions with a focus on premium aesthetics and performance.</p>
                     </div>
                  ))}
               </div>
            </div>
         ...
         <section id="projects" style={{ padding: '10rem 6%' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
               <h2 style={{ fontSize: '3.5rem', fontWeight: 900, marginBottom: '6rem', textAlign: 'center' }}>Elite <span style={{ color: primaryColor }}>Portfolio</span></h2>
                  ...
                  {data.projects?.map((p, i) => (
                     <div key={i} className="elite-card">
                        ...
                        <div style={{ display: 'flex', gap: '0.8rem', flexWrap: 'wrap', marginBottom: '3rem' }}>
                           {p.tech?.split(',').map(t => <span key={t} style={{ background: primaryColor, color: '#0B0B0B', padding: '0.5rem 1rem', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 700 }}>{t}</span>)}
                        </div>
                        <button className="cta-outline" style={{ width: '100%' }} onClick={() => window.open(p.link)}>View Project</button>
                     </div>
                  ))}
               </div>
            </div>
         ...
         <section id="experience" style={{ padding: '10rem 6%' }}>
            <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
               <h2 style={{ fontSize: '3.5rem', fontWeight: 900, marginBottom: '8rem', textAlign: 'center' }}>Professional <span style={{ color: primaryColor }}>Timeline</span></h2>
               {data.experience?.map((e, i) => (
                  <div key={i} style={{ display: 'flex', gap: '4rem', marginBottom: '6rem', paddingLeft: '3rem', borderLeft: `2px solid ${primaryColor}44`, position: 'relative' }}>
                     ...
                     <div className="elite-card" style={{ padding: '2.5rem', flex: 1 }}>
         ...
         <section id="skills" style={{ padding: '10rem 6%' }}>
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
               <h2 style={{ fontSize: '4rem', fontWeight: 900, marginBottom: '6rem', textAlign: 'center', color: primaryColor }}>Technical <span style={{ color: isDarkMode ? '#fff' : '#111827' }}>Mastery</span></h2>
               <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
                  {(() => {
                     const skillCategories: Record<string, string[]> = {
                        "Modern Stack": ["JavaScript", "TypeScript", "React", "Next.js", "Node.js", "Python"],
                        "Engineering": ["AWS", "Docker", "Kubernetes", "PostgreSQL", "NoSQL", "System Architecture"],
                        "Product": ["UI/UX Design", "Product Strategy", "Growth Hacking", "SEO Management"],
                     };
                     ...
                     return renderedCategories.map((c, i) => (
                        <div key={i} className="skill-grid-card">
                           ...
                           <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                              {c.skills.map(s => (
                                 <span key={s} style={{ padding: '0.8rem 1.8rem', background: isDarkMode ? 'rgba(0,0,0,0.3)' : '#f3f4f6', borderRadius: '100px', fontWeight: 700, fontSize: '1rem', border: `1px solid ${primaryColor}22` }}>
                                    {s}
                                 </span>
                              ))}
                           </div>
                        ...
                        <div key={i} className="elite-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
                           <div>
                              <div style={{ color: primaryColor, marginBottom: '1.5rem' }}>
                                 <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87L18.18 21 12 17.27 5.82 21 7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg>
                              </div>
         ...
         <section id="contact" style={{ padding: '10rem 6%' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
               <h2 style={{ fontSize: '3.5rem', fontWeight: 900, textAlign: 'center', marginBottom: '8rem', color: primaryColor }}>Get In <span style={{ color: isDarkMode ? '#fff' : '#111827' }}>Touch</span></h2>
                  ...
                  <div className="elite-card">
                     ...
                     <form action={`https://formsubmit.co/${data.email}`} method="POST" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <div>
                           ...
                           <input type="text" name="name" required placeholder="Your Name" style={{ width: '100%', padding: '1.2rem', borderRadius: '12px', background: isDarkMode ? 'rgba(0,0,0,0.2)' : '#f3f4f6', border: 'none', color: 'inherit', fontWeight: 600 }} />
                        </div>
                        <div>
                           <label style={{ fontSize: '0.9rem', fontWeight: 800, opacity: 0.8, display: 'block', marginBottom: '0.6rem' }}>Email</label>
                           <input type="email" name="email" required placeholder="Your Email" style={{ width: '100%', padding: '1.2rem', borderRadius: '12px', background: isDarkMode ? 'rgba(0,0,0,0.2)' : '#f3f4f6', border: 'none', color: 'inherit', fontWeight: 600 }} />
                        </div>
                        <div>
                           <label style={{ fontSize: '0.9rem', fontWeight: 800, opacity: 0.8, display: 'block', marginBottom: '0.6rem' }}>Message</label>
                           <textarea name="message" rows={5} required placeholder="Your Message" style={{ width: '100%', padding: '1.2rem', borderRadius: '12px', background: isDarkMode ? 'rgba(0,0,0,0.2)' : '#f3f4f6', border: 'none', color: 'inherit', fontWeight: 600 }}></textarea>
                        </div>
                        <button type="submit" className="cta-primary" style={{ width: '100%', marginTop: '1rem' }}>Send Message</button>
                     </form>
                  </div>

                  <div className="elite-card" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                     ...
                     <div style={{ marginTop: 'auto', borderRadius: '20px', overflow: 'hidden', border: `1px solid ${primaryColor}22`, height: '230px' }}>
                        <iframe 
                           width="100%" 
                           height="100%" 
                           frameBorder="0" 
            ...
            <p style={{ marginTop: '10rem', opacity: 0.25, textAlign: 'center', fontSize: '0.8rem', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 900, userSelect: 'none', pointerEvents: 'none' }}>
               © {new Date().getFullYear()} @{data.name.toLowerCase().replace(/\s+/g, '')}. Created with <span style={{ color: primaryColor }}>PortGen 3D Elite.</span>
            </p>
         </section>
      </div>
   );
