...
const Card4: React.FC<TemplateProps> = ({ data }) => {
  ...
  return (
    <div style={{ background: '#000000', color: '#ffffff', minHeight: '100vh', fontFamily: "'Inter', sans-serif", scrollBehavior: 'smooth' }}>
      ...
      {/* Navigation */}
      <nav style={{ padding: '1.5rem 6%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#000000da', backdropFilter: 'blur(16px)', position: 'sticky', top: 0, zIndex: 100, borderBottom: '1px solid #111' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', cursor: 'pointer' }} onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
          <div className="logo-box" style={{ 
            width: '45px', height: '45px', background: 'linear-gradient(135deg, #ef4444 0%, #f43f5e 100%)', 
            borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '1.8rem', fontWeight: 950 
        ...
        <div style={{ display: 'flex', gap: '2.5rem', alignItems: 'center' }}>
          ...
          <a href="#expertise" style={{ color: '#fff', opacity: 0.6, fontWeight: 700, textDecoration: 'none' }}>Expertise</a>
          <a href="#journey" style={{ color: '#fff', opacity: 0.6, fontWeight: 700, textDecoration: 'none' }}>Experience</a>
          <a href="#certifications" style={{ color: '#fff', opacity: 0.6, fontWeight: 700, textDecoration: 'none' }}>Certifications</a>
      ...
      {/* Expertise */}
      <section id="expertise" style={{ padding: '10rem 6%', background: '#0a0a0a' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {data.certificates?.length > 0 && (
        <section id="certifications" style={{ padding: '10rem 6%', background: '#0a0a0a' }}>
          <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
            <h3 style={{ fontSize: '3.5rem', fontWeight: 950, textAlign: 'center', marginBottom: '6rem' }}>Global <span className="gradient-text">Certifications</span></h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '3rem' }}>
              {data.certificates.map((cert, i) => (
                <a key={i} href={cert.url} target="_blank" rel="noopener noreferrer" className="premium-card" style={{ padding: '4rem', textDecoration: 'none', background: '#111' }}>
                  <h4 style={{ fontSize: '1.6rem', color: '#fff', fontWeight: 950, marginBottom: '1rem' }}>{cert.name}</h4>
                  <p style={{ color: '#ef4444', fontWeight: 900, fontSize: '1rem' }}>{cert.issuer}</p>
                  <p style={{ color: '#444', fontSize: '0.9rem', marginTop: '2.5rem' }}>AUTHENTICATED ➔</p>
                </a>
              ))}
            </div>
          </div>
